 ...
 *     + platformMatch    * 0.05
 *
 * Each component is 0..1; the final score is normalised to 0..100.
 *
 * Candidate generation never scans the whole dataset: candidates come from the
 * inverted index (shared terms), the category/subcategory postings and the
 * host postings. That keeps Find Similar O(candidates) instead of O(n).
 */
(function (root) {
  'use strict';
  const FMHYS = root.FMHYS;
  ...
  /** Below this normalised score a candidate is not a "genuine" alternative. */
  const MIN_SCORE = 28;
  const MAX_RESULTS = 5;
  ...
  function categoryScore(a, b) {
    if (a.category !== b.category) {
      // Related-but-different categories still share tokens ("Video" / "Video Tools").
      const ta = new Set(T.terms(a.category));
      const tb = new Set(T.terms(b.category));
      return T.overlap(ta, tb) * 0.4;
    }
  ...
  function listScore(a, b) {
    if (!a.length || !b.length) return 0;
    return 0.5 * T.jaccard(a, b) + 0.5 * T.overlap(a, b);
  }

  function platformScore(a, b) {
    const pa = (a.platforms || []).map((s) => s.toLowerCase());
    const pb = (b.platforms || []).map((s) => s.toLowerCase());
    if (!pa.length || !pb.length) return 0.5; // unknown: neutral, not punitive
    return T.overlap(pa, pb);
  }

  /**
   * Cosine-ish similarity over description terms with light IDF weighting.
   */
  function textScore(termsA, termsB, idf) {
    if (!termsA.size || !termsB.size) return 0;
    let dot = 0, na = 0, nb = 0;
    for (const t of termsA) { const w = idf(t); na += w * w; if (termsB.has(t)) dot += w * w; }
    for (const t of termsB) { const w = idf(t); nb += w * w; }
  ...
  class SimilarityEngine {
    /**
     * @param {FMHYS.search.SearchIndex} index
     */
    constructor(index) {
      this.index = index;
      this.cache = new Map();       // resourceId -> results
      this.cacheOrder = [];
      this.cacheLimit = 300;
      this._idfCache = new Map();
      this._descTerms = null;
    }
    ...
    _descTermsFor(i) {
      if (!this._descTerms) this._descTerms = new Array(this.index.size);
      let s = this._descTerms[i];
      if (!s) {
        const d = this.index.docs[i];
        s = new Set(T.terms((d.description || '') + ' ' + d.name));
        this._descTerms[i] = s;
      }
      return s;
    }

    /** Collect a bounded candidate pool from the index postings. */
    _candidates(srcIdx) {
      ...
      const addAll = (arr) => {
        ...
        for (const i of arr) {
          ...
          pool.add(i);
          if (pool.size >= MAX_CANDIDATES) return;
        }
      };

      // 1. same subcategory (strongest signal)
      if (src.subcategory) addAll(this.index.bySubcategory.get(src.category + '\u0000' + src.subcategory));
      // 2. shared distinctive terms (tags, name, description)
      const srcTerms = this.index.docTerms[srcIdx];
      if (srcTerms) {
        const ranked = Array.from(srcTerms).sort((a, b) => this._idf(b) - this._idf(a));
        for (const t of ranked) {
          if (pool.size >= MAX_CANDIDATES) break;
          const postings = this.index.termIndex.get(t);
          // Skip ultra-common terms; they add noise, not signal.
          if (!postings || postings.length > 1500) continue;
          addAll(postings);
        }
      }
      // 3. same category, as a floor
      if (pool.size < 40) addAll(this.index.byCategory.get(src.category));

      return pool;
    }

    /**
     * Detailed score breakdown for a pair.
    ...
    score(srcIdx, candIdx) {
      ...
      const parts = {
        ...
        tags: listScore(a.tags || [], b.tags || []),
        keywords: textScore(this.index.docTerms[srcIdx], this.index.docTerms[candIdx], (t) => this._idf(t)),
        description: textScore(this._descTermsFor(srcIdx), this._descTermsFor(candIdx), (t) => this._idf(t)),
        features: listScore(a.features || [], b.features || []),
        platform: platformScore(a, b)
      };
      ...
      let raw =
        ...
        parts.features * WEIGHTS.features +
        parts.platform * WEIGHTS.platform;

      // Same resource type is a mild confirmation, not a weighted axis.
      if (a.type && b.type && a.type === b.type) raw *= 1.06;
      // FMHY-recommended alternatives get a small, transparent nudge.
      if (b.starred) raw *= 1.05;
      // Same registrable domain = usually the same product; not an alternative.
      if (U.registrableDomain(a.url) && U.registrableDomain(a.url) === U.registrableDomain(b.url)) {
        raw *= 0.25;
      }
     ...
     * @returns {Array<{resource:object, score:number, parts:object}>}
     */
    findSimilar(resourceId, opts) {
      ...
      for (const candIdx of this._candidates(srcIdx)) {
        ...
        scored.push({ resource: cand, score, parts, _idx: candIdx });
      }
      ...
      // Dedupe by domain and by name so the list shows 5 *distinct* options.
      const out = [];
  ...
  FMHYS.similarity = { SimilarityEngine, WEIGHTS, MIN_SCORE, MAX_RESULTS };
})(typeof globalThis !== 'undefined' ? globalThis : self);
